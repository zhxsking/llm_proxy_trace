...
本文件为 AI Agent（Claude、CodeX 等）提供项目上下文，帮助快速理解代码库并减少误操作。
...
4. **TypeScript 构建**：`tsconfig.json` 的 `moduleResolution: bundler` 模式，导入路径需用 `.js` 扩展名（即使源文件是 `.ts`）。

5. **无 react-router**：前端无路由框架，所有状态在 `App.tsx`，组件树：App → TraceSidebar + TraceDetail + SettingsPanel（浮层）。
...
- **使用中文提交注释**，要精炼
- 有较大更新时自动添加tag，但**不要**自动git push和npm publish，除非用户明确要求
