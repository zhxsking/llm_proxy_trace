...
**LPT（LLM Proxy Trace）** 是一个本地 LLM 代理工具，将请求从 LLM 客户端透明转发到真实 Provider（OpenAI / Anthropic / Ollama 等），同时捕获完整 Trace 并通过 Web 面板实时展示。
...
1. **路由顺序**：`/api/chat`（Ollama）和 `/api/*`（管理）共用 `/api` 前缀，`app.ts` 中管理路由用 `app.route('/api', apiRoutes)` 注册，Ollama 的 `/api/chat` 直接挂载，顺序敏感。
